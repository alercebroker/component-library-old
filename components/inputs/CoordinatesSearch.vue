<template>
  <v-layout wrap align-center>
    ...
    <v-flex xs10 sm12 md12>
      <v-row>
        <v-col lg="6" md="12">
          <v-text-field v-model="targetName" label="Target Name" :error-messages="error" />
        </v-col>
        <!--BOTON-->
    ...
    <!--RA-->
    <v-flex xs5>
      <v-text-field v-model="localValue.ra" type="number" step="0.00001" label="RA (deg)"
        :error-messages="validationErrors.conesearch" />
    </v-flex>
    <v-spacer />
    <!--DEC-->
    <v-flex xs5>
      <v-text-field v-model="localValue.dec" label="DEC (deg)" type="number" step="0.00001"
        :error-messages="validationErrors.conesearch" />
    </v-flex>
    <!--RADIUS-->
    <v-flex xs12>
